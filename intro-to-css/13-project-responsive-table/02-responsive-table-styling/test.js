test("table wrapper allows horizontal scrolling", () => {
  const css = document.querySelector("style").textContent;
  assert.match(css, /\.table-wrap\s*\{[^}]*overflow-x\s*:\s*auto\s*;/i, "Set .table-wrap overflow-x to auto");
});

test("table cells have padding and borders", () => {
  const css = document.querySelector("style").textContent;
  assert.match(css, /th\s*,\s*td\s*\{[^}]*padding\s*:/i, "Add padding styles for th and td");
  assert.match(css, /th\s*,\s*td\s*\{[^}]*border\s*:/i, "Add border styles for th and td");
});

test("media query exists for small screens", () => {
  const css = document.querySelector("style").textContent;
